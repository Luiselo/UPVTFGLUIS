...
const numColumns = Math.floor(dims.width / columnWidth);
  
// Calcula el desplazamiento horizontal para centrar las columnas
const totalWidth = numColumns * columnWidth;
const offsetX = (dims.width*2 - totalWidth) / 2;

const groupedDataArray = Object.entries(groupedData);

groupedDataArray.forEach(([curso, group], groupIndex) => {
  group.forEach((d, i) => {
    d.x = (groupIndex % numColumns) * columnWidth + offsetX; // Index de columna con desplazamiento
    d.y = Math.floor(groupIndex / numColumns) * height + i * rowHeight; // Index de fila
  });
});
...
label.transition()
    .delay((d, i) => i * 20).text(d => console.log('meep meep',d))
...
overlay.transition(t)
  ...
  function updateLista()
  {
    ...
    .duration(750);    
  label.transition(t)
    .delay((d, i) => i * 20).style("font-size", "10px")
    .attrTween("transform", d => {
...
const svgWidth = parseInt(d3.select("svg").style("width")) - margin.left - margin.right;
const svgHeight = parseInt(d3.select("svg").style("height")) - margin.top - margin.bottom;

// Ajusta tus escalas según las dimensiones del SVG
...
// Transiciones y actualizaciones
label.transition(t)
  .delay((d, i) => i * 20)
  .attrTween("transform", d => {
    const i = d3.interpolateNumber(d.y, y(d.id));
...
// Ajuste de los elementos según el contenedor SVG con márgenes
d3.selectAll(".bar")
...
async function InicializarGrafo(db){
        ...
        console.log('InicializarGrafo',data,db);
...
function imprimirTimeLine(aux, svg, color, relaciones) {
  ...
  console.log('esto es svg', svg)
  console.log('esto es color',color)
  console.log('esto es relaciones',relaciones)
  const expandirButton = document.getElementById('expandir-button');
  let izquierdoVisible = true;
  disperseGrafico();

  // Eliminar los elementos existentes del gráfico
  d3.select('.canvas').select('svg').selectAll("*").remove();

  // Verificar si nodesOrdenados es un array válido
  if (!Array.isArray(aux)) {
    console.error("nodesOrdenados no es un array válido");
    return;
  }

  // Filtrar nodos que no tengan un grupo definido
  const nodesOrdenados = aux.filter(d => d.group !== undefined);

  // Configurar nuevamente el gráfico con los nodos ordenados por grupo
  const dims = { height: 700, width: 500 };
  ...
  const y = d3.scalePoint()
    .domain(nodesOrdenados.map(d => d.id).sort(d3.ascending))
    .range([margin.top, dims.height - margin.bottom]);

  const label = svgCanvas.append("g")
    .attr("font-family", "sans-serif")
    .attr("font-size", 8)
    ...
    .enter()
    .append("g")
    .attr("transform", d => `translate(${xPosition(d.group)}, ${y(d)})`);
  ...
  const overlay = svgCanvas.append("g")
    .attr("fill", "none")
    .attr("pointer-events", "all")
    .selectAll("rect")
    .data(nodesOrdenados)
    .enter()
    .append("rect")
    .attr("width", columnaWidth)
    .attr("height", step)
    .attr("y", d => y(d.id) - step / 2)
    .attr("x", d => xPosition(d.group) - columnaWidth / 2)
    .on("mouseover", function (d) {
      svgCanvas.classed("hover", true);
      label.classed("primary", n => n === d);
      label.classed("secondary", n => n.sourceLinks.some(l => l.target === d) || n.targetLinks.some(l => l.source === d));
      path.classed("primary", l => l.source === d || l.target === d).filter(".primary").raise();
    }).on("mouseout", d => {
      svgCanvas.classed("hover", false);
      label.classed("primary", false);
      label.classed("secondary", false);
      path.classed("primary", false).order();
    });

  console.log("Nodos ordenados por grupo:", nodesOrdenados);
}
