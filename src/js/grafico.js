...
const botonesActivos = obtenerBotonesActivos();
  ...
  if(tagsFiltro[0] !== "-1"){
    ...
    asignaturas2 = data.flatMap(d => d.asignaturas.filter(a => botonesActivos.includes(a.cursoID)))
      asignaturasFiltradas = data.flatMap(d => d.asignaturas.filter(asignatura => {
  ...
  // Mapear los nombres utilizando la función obtenerNombrePorIdioma
// Mapear los nombres utilizando flatMap y la función obtenerNombrePorIdioma
...
async function obtenerGrafo(id){
  try {
      ...
      const url = `http://localhost/UpTask_MVC/public/index.php/api/grafo?id=${proyecto.id}`;
...
toggleButton.addEventListener('click', () => {
  ...
  if (navbar.classList.contains('hidden')) {
    ...
    setTimeout(() => {
      navbar.style.opacity = 1; // Asegura que la opacidad sea 1 después de mostrar
    }, 10); // Añade un pequeño retraso para que el cambio de opacidad ocurra después de que la clase se elimine
  } else {
  
    navbar.style.opacity = 0; // Establece la opacidad a 0 para desvanecer
    setTimeout(() => {
      navbar.classList.add('hidden');
    }, 500); // Espera el tiempo de la transición antes de ocultar completamente
  }
});
